/* Codebase Atlas engine — isometric hatched drafting-paper map.
   Reads window.ATLAS_DATA; registers <codebase-atlas paper="tan|light|dark" flow="true|false">. */
(function () {
  ...
  const P = (gx, gy, h) => [(gx - gy) * SX, (gx + gy) * SY - (h || 0) * SH];
  ...
  class Atlas extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (window.ATLAS_DATA) boot();
      else { const iv = setInterval(() => { if (window.ATLAS_DATA) { clearInterval(iv); boot(); } }, 50); }
    }
    disconnectedCallback() { this.dead = true; if (this.onKey) window.removeEventListener('keydown', this.onKey); }
    attributeChangedCallback(n, a, b) { if (a === b || !this.D) return; if (n === 'paper') this.build(); }
    theme() { return THEMES[this.getAttribute('paper')] || THEMES.tan; }
    ...
    build() {
      ...
      const cell = (k, v) => el('div', `padding:8px 14px;border-right:1.5px solid ${T.ink};display:flex;flex-direction:column;justify-content:space-between;flex:none`,
    ...
    renderScene() {
      ...
      const sById = {}; structs.forEach((s) => { sById[s.id] = s; });
      const ctr = (s) => [s.gx + s.w / 2, s.gy + s.d / 2];
      ...
      structs.slice().sort((a, b) => (a.gx + a.gy + (a.w + a.d) / 2) - (b.gx + b.gy + (b.w + b.d) / 2)).forEach((s) => {
        const { gx, gy, w, d, h } = s;
        const Ag = P(gx, gy, 0), Bg = P(gx + w, gy, 0), Cg = P(gx + w, gy + d, 0), Dg = P(gx, gy + d, 0);
        ...
        const lp = [(Dg[0] + Cg[0]) / 2, Math.max(Cg[1], Dg[1]) + 11];
    ...
    tip(ev, html) { this.tipEl.innerHTML = html; this.tipEl.style.display = 'block'; this.tipMove(ev); }
    tipMove(ev) {
    ...
    bindPanZoom() {
      ...
      let drag = null;
      svg.addEventListener('pointerdown', (e) => { drag = { x: e.clientX, y: e.clientY, vb: this.vb.slice(), moved: false }; svg.setPointerCapture(e.pointerId); svg.style.cursor = 'grabbing'; });
      ...
      svg.addEventListener('pointerup', (e) => {
        ...
        if (!wasDrag && (e.target === svg || e.target.tagName === 'defs')) { this.sel = null; if (this.traceI >= 0) this.endTrace(); else this.syncUI(); }
      });
    }
    loop(t) {
      if (this.dead) return;
    ...
    paintSel() {
      ...
      if (this.rowEls) for (const id in this.rowEls) {
        ...
        r.querySelector('span').style.borderColor = on ? T.bg : T.ink;
      }
    }
    setHash(h) { try { history.replaceState(null, '', location.pathname + location.search + (h || '#')); } catch (e) {} }
    goInside(id, silent) {
    ...
    stepTrace(d) { this.traceI = Math.max(0, Math.min(this.D.TRACE.length - 1, this.traceI + d)); this.applyTrace(); }
    ...
    // ───────────────────────── right panel ─────────────────────────
    rich(t) { const T = this.theme(); return esc(t).replace(/\[\[(.+?)\]\]/g, `<span style="background:${T.ink};color:${T.bg};padding:0 4px">$1</span>`); }
    hRule(label) { const T = this.theme(); return `<div style="display:flex;align-items:center;gap:9px;margin:20px 0 9px"><span style="font-size:9px;letter-spacing:.18em;white-space:nowrap">${label}</span><span style="flex:1;height:1.5px;background:${T.ink}"></span></div>`; }
    renderPanel() {
      const T = this.theme(), D = this.D, P = this.panelEl;
      ...
      if (this.traceI >= 0) {
        ...
        P.querySelector('#tPrev').onclick = () => this.stepTrace(-1);
        P.querySelector('#tNext').onclick = () => this.stepTrace(1);
        P.querySelector('#tEnd').onclick = () => this.endTrace();
        return;
      }
      const cur = this.sel && (this.inside ? this.structsNow().find((x) => x.id === this.sel) : this.byId[this.sel]);
      if (cur) {
        ...
        P.querySelector('#pBack').onclick = () => { this.sel = null; this.syncUI(); };
        const pin = P.querySelector('#pIn'); if (pin) pin.onclick = () => this.goInside(cur.id);
        P.querySelectorAll('#pTalks button').forEach((b) => { b.onclick = () => this.select(b.dataset.id); });
        return;
      }
      // overview
      P.innerHTML = `
      ...
      P.querySelector('#pTrace').onclick = () => this.startTrace();
    }
  }
